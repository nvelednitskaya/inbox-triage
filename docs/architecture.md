...
- **Notifications are limited to recent mail.** Sorting is retroactive — the
  first live run works through whatever is already in the inbox — but a
  notification for a three-week-old email is noise, not a signal. Only mail
  newer than a few hours triggers Telegram; everything older is labeled
  silently.
